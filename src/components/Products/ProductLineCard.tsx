  ...
  return (
    <Reveal delay={delay} className="h-full w-full">
      <button
        ...
        aria-pressed={active}
        className={`group relative flex h-full min-h-[320px] w-full flex-col overflow-hidden rounded-[24px] text-start outline-none transition-[transform,box-shadow] duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-1 focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 sm:min-h-[360px] ${
          active
      ...
      >
        ...
        <span className="relative z-[1] mt-auto flex flex-col gap-2.5 p-5 sm:p-6">
          <span className="h-0.5 w-8 origin-start scale-x-0 bg-brand transition-transform duration-500 group-hover:scale-x-100" />
          <span className="text-[1.2rem] font-bold leading-[1.25] text-white sm:text-[1.3rem]">
            {title}
          </span>
          <span className="line-clamp-2 text-[0.9rem] leading-[1.65] text-white/75">
            {description}
          </span>
          <span className="mt-1 inline-flex w-fit items-center text-[0.8rem] font-semibold uppercase tracking-[0.08em] text-brand opacity-0 transition-opacity duration-300 group-hover:opacity-100 group-focus-visible:opacity-100">
            {inquireLabel}
          </span>
        </span>
      </button>
